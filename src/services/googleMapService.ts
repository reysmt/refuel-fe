import { axiosGmapTileReqCheck, axiosGmapInstance, axiosGmapReverseGeocoding } from "./axiosService";
...
async function getReverseGeocoding(lat, lng, key) {
  return await axiosGmapReverseGeocoding(lat, lng, key).get('').then(resp => resp.data).catch(
    function (error){
      if (error.response) {
        // The request was made and the server responded with a status code
        // that falls out of the range of 2xx
